...
// import { Activity, AlertTriangle, BarChart2, Map, Play } from "lucide-react";

// import { EarningsChart } from "@/components/charts/EarningsChart";
// import type { EarningsLog } from "@/lib/types";

// type TabId = "events" | "trips" | "earnings";

// type Severity = "safe" | "low" | "medium" | "high";

// interface LatestFlag {
//   timestamp: string;
//   tripId: string;
...
// interface TripSummaryLite {
//   tripId: string;
//   flags: number;
//   worstSeverity: Severity;
// }

// export default function DriverPulseDemo() {
//   const [activeTab, setActiveTab] = useState<TabId>("events");
//   const [isRunning, setIsRunning] = useState(false);
//   const [latestFlag, setLatestFlag] = useState<LatestFlag | null>(null);
//   const [tripSummaries, setTripSummaries] = useState<Record<string, TripSummaryLite>>(
//     {},
//   );
//   const [earningsLog, setEarningsLog] = useState<EarningsLog[]>([]);
...
//     const es = new EventSource(`${backendBase}/api/sensor/demo?interval=1`);
//     eventSourceRef.current = es;
//     setIsRunning(true);

//     es.addEventListener("demo_update", (ev) => {
//       const data = JSON.parse((ev as MessageEvent).data);

//       // Update latest flagged event (only if severity is not safe)
//       const severity: Severity = data.fusion?.severity ?? "safe";
//       if (severity !== "safe") {
//         setLatestFlag({
//           timestamp: data.timestamp,
//           tripId: data.trip_id,
//           flagType: data.fusion?.flag_type ?? null,
//           severity,
//           speedKmh: data.speed_kmh,
//           motionType: data.motion?.event_type ?? "normal",
//           audioClass: data.audio?.classification ?? "background",
//         });

//         setTripSummaries((prev) => {
//           const existing = prev[data.trip_id];
//           const currentWorst = existing?.worstSeverity ?? "safe";
//           const rank: Record<Severity, number> = { safe: 0, low: 1, medium: 2, high: 3 };
//           const worstSeverity =
//             rank[severity] > rank[currentWorst] ? severity : currentWorst;

//           return {
//             ...prev,
//             [data.trip_id]: {
//               tripId: data.trip_id,
//               flags: (existing?.flags ?? 0) + 1,
//               worstSeverity,
//             },
//           };
//         });
...
//       // Track latest earnings status for the status view
//       if (data.earnings?.status) {
//         setLatestStatus(data.earnings.status);
//       }
//     });

//     // Per-trip earnings summaries (one event at end of each trip)
//     es.addEventListener("trip_done", (ev) => {
//       const data = JSON.parse((ev as MessageEvent).data);
...
//   const tripsList = Object.values(tripSummaries).sort((a, b) =>
...
//             disabled={isRunning}
//             className={[
//               "inline-flex items-center gap-2 rounded-xl px-4 py-2 text-[13px] font-semibold",
//               "transition-all duration-200 min-h-[40px]",
//               isRunning
//                 ? "bg-[#1a1a1a] text-[#777] cursor-not-allowed"
//                 : "bg-[#276EF1] text-white hover:bg-[#2f7bff] active:scale-95",
//             ].join(" ")}
//           >
...
//       {/* Tabs */}
//       <nav className="border-b border-[#141414] bg-[#050505] px-4 lg:px-6 py-2">
//         <div className="flex max-w-screen-xl mx-auto gap-2 text-[11px] font-semibold">
//           <TabButton
//             id="events"
//             icon={AlertTriangle}
//             label="Latest Flagged Event"
//             activeTab={activeTab}
...
//           {activeTab === "events" && <EventsTab latestFlag={latestFlag} />}
//           {activeTab === "trips" && <TripsTab trips={tripsList} />}
//           {activeTab === "earnings" && (
//             <EarningsTab earningsLog={earningsLog} latestStatus={latestStatus} />
//           )}
//         </div>
//       </main>
//     </div>
...
// function EventsTab({ latestFlag }: { latestFlag: LatestFlag | null }) {
//   if (!latestFlag) {
//     return (
//       <div className="mt-8 rounded-2xl border border-[#1f1f1f] bg-[#0b0b0b] p-6 text-sm text-[#777]">
//         When you tap <span className="font-semibold text-white">Run Demo</span>, your
//         most recent safety or conflict event will appear here in real time.
//       </div>
//     );
//   }
...
//       <div className="rounded-2xl border border-[#1f1f1f] bg-[#0b0b0b] p-5 space-y-3">
//         <div className="flex items-center justify-between gap-3">
//           <div className="flex items-center gap-2.5">
//             <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#E54937]/10">
//               <AlertTriangle
//                 size={16}
//                 className={severityColor[latestFlag.severity]}
//                 aria-hidden
//               />
//             </div>
//             <div>
//               <p className="text-[11px] font-semibold text-[#888] uppercase tracking-[0.15em]">
//                 Latest Flagged Event
//               </p>
//               <p className="text-sm font-semibold text-white">
//                 {latestFlag.flagType || "Driving event"}
//               </p>
//             </div>
//           </div>
//           <span className={`text-[11px] font-semibold ${severityColor[latestFlag.severity]}`}>
//             {latestFlag.severity.toUpperCase()}
//           </span>
//         </div>

//         <div className="flex items-center justify-between text-[11px] text-[#777]">
//           <span>
//             Trip <span className="font-mono text-white">{latestFlag.tripId}</span>
//           </span>
//           <span>{new Date(latestFlag.timestamp).toLocaleTimeString()}</span>
//         </div>

//         <div className="mt-2 grid grid-cols-3 gap-3 text-[11px] text-[#999]">
//           <div>
//             <p className="text-[#666]">Speed</p>
//             <p className="mt-0.5 text-sm font-semibold text-white">
//               {latestFlag.speedKmh.toFixed(0)} km/h
//             </p>
//           </div>
//           <div>
//             <p className="text-[#666]">Motion</p>
//             <p className="mt-0.5 text-sm font-semibold text-white capitalize">
//               {latestFlag.motionType.replace("_", " ")}
//             </p>
//           </div>
//           <div>
//             <p className="text-[#666]">Cabin audio</p>
//             <p className="mt-0.5 text-sm font-semibold text-white capitalize">
//               {latestFlag.audioClass.replace("_", " ")}
//             </p>
//           </div>
//         </div>
//       </div>
...
// function TripsTab({ trips }: { trips: TripSummaryLite[] }) {
//   if (!trips.length) {
//     return (
//       <div className="mt-8 rounded-2xl border border-[#1f1f1f] bg-[#0b0b0b] p-6 text-sm text-[#777]">
//         Trip summaries will appear here as the demo completes each trip.
//       </div>
//     );
//   }
...
//   return (
//     <div className="mt-6 overflow-hidden rounded-2xl border border-[#1f1f1f] bg-[#050505]">
//       <table className="min-w-full text-left text-[11px]">
//         <thead className="bg-[#0d0d0d] text-[#777] uppercase tracking-[0.14em]">
//           <tr>
//             <th className="px-4 py-3 font-semibold">Trip</th>
//             <th className="px-4 py-3 font-semibold">Flags</th>
//             <th className="px-4 py-3 font-semibold">Worst Severity</th>
//           </tr>
//         </thead>
//         <tbody>
//           {trips.map((t) => (
//             <tr key={t.tripId} className="border-t border-[#151515] text-[12px]">
//               <td className="px-4 py-2.5 font-mono text-[#ddd]">{t.tripId}</td>
//               <td className="px-4 py-2.5">{t.flags}</td>
//               <td className="px-4 py-2.5">
//                 <span className="inline-flex rounded-full bg-white/5 px-2 py-0.5 text-[11px] text-[#ccc]">
//                   {severityLabel[t.worstSeverity]}
//                 </span>
//               </td>
//             </tr>
//           ))}
...
//   latestStatus,
// }: {
//   earningsLog: EarningsLog[];
//   latestStatus: string | null;
// }) {
//   if (!earningsLog.length) {
//     return (
//       <div className="mt-8 rounded-2xl border border-[#1f1f1f] bg-[#0b0b0b] p-6 text-sm text-[#777]">
//         Start the demo to see your earnings curve build up during the shift.
//       </div>
//     );
//   }

//   const latest = earningsLog[earningsLog.length - 1];
...
//     ahead: "text-[#05C168]",
//     on_track: "text-[#FFC043]",
//     at_risk: "text-[#E54937]",
//   };
//   const statusText = latestStatus && statusLabel[latestStatus] ? statusLabel[latestStatus] : null;

//   return (
//     <div className="mt-6 space-y-4">
//       <div className="rounded-2xl border border-[#1f1f1f] bg-[#050505] p-5">
//         <div className="flex items-center justify-between gap-2 mb-3">
//           <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#666]">
//             Earnings so far
//           </p>
//           <span className="text-[11px] text-[#777]">{latest.timestamp}</span>
//         </div>
//         <div className="flex items-baseline gap-2 mb-4">
//           <span className="text-[40px] font-extrabold leading-none tracking-tight text-white">
//             ₹{latest.earnings.toFixed(0)}
//           </span>
//           {statusText && (
//             <span
//               className={`ml-2 text-[11px] font-semibold uppercase tracking-[0.14em] ${statusColor[latestStatus!] ?? "text-[#777]"}`}
//             >
//               {statusText}
//             </span>
//           )}
...
/**
 * DriverPulse — fully backend-driven, zero hardcoded business values.
 *
 * Every number, threshold, status string, driver name, city, target, currency
 * symbol, trip code, fare, distance — ALL come from the SSE payload emitted by
 * stream_demo_events() in demo.py.
 *
 * The mock at the bottom (simulateDemoStream) reproduces the exact payload
 * schema from stream_demo_events so the component works standalone in an
 * artifact. In production, swap the mock call for:
 *
 *   const es = new EventSource(`${BACKEND}/api/sensor/demo?interval=1`);
 *   es.addEventListener("demo_update", ev => onUpdate(JSON.parse(ev.data)));
 *   es.addEventListener("trip_done",   ev => onTripDone(JSON.parse(ev.data)));
 *   es.addEventListener("done",        ev => onDone(JSON.parse(ev.data)));
 *
...
const Card = ({ children, style, ...p }) => (
  <div style={{ background:"#0f0f0f", border:"1px solid #1e1e1e",
    borderRadius:18, padding:"18px 20px", ...style }} {...p}>
    {children}
  </div>
);
...
const MiniStat = ({ label, value, color = "#d0d0d0" }) => (
  <div style={{ background:"#141414", borderRadius:10, padding:"10px 12px" }}>
    <div style={{ fontSize:10, color:"#555", textTransform:"uppercase",
      letterSpacing:"0.13em", fontFamily:"monospace" }}>{label}</div>
...
export default function DriverPulseApp() {
  ...
  const handleUpdate = useCallback((data) => {
    ...
    setFlagged(prev => [{
      ...
      uploadTier:      data.fusion.upload_tier,
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#060606", color:"#e8e8e8",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <div style={{ fontSize:17, fontWeight:800, color:"#fff",
              fontFamily:"Georgia,serif" }}>Driver Pulse</div>
            <div style={{ fontSize:10, color:"#444", letterSpacing:"0.16em",
      ...
      <nav style={{ borderBottom:"1px solid #141414", background:"#080808",
...
function Dashboard({ sd, pct, currentE, projected, target, cur,
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <Card style={{ background:`linear-gradient(135deg,${sd.color}15,#0f0f0f)`,
        borderColor:`${sd.color}30` }}>
        <Lbl>Shift Status</Lbl>
        <div style={{ display:"flex", alignItems:"center",
          justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
          <div style={{ fontSize:44, fontWeight:900, color:sd.color,
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      <Card style={{ padding:"13px 16px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
          ...
          {hasFilter && (
            <button onClick={clearAll} style={{ padding:"7px 12px", borderRadius:8,
          ...
          <Card key={t.tripId} style={{ borderColor:t.flags ? c.border : "#1e1e1e" }}>
            <div style={{ display:"flex", alignItems:"center",
              ...
              <div>
                <div style={{ fontSize:24, fontWeight:800, color:"#fff",
                  fontFamily:"Georgia,serif" }}>{t.tripId}</div>
                {t.completedAt && (
                  <div style={{ fontSize:12, color:"#555", fontFamily:"monospace" }}>
            ...
            {(uniqMotion.length > 0 || uniqAudio.length > 0) && (
              <div style={{ marginTop:12, display:"flex", gap:6, flexWrap:"wrap" }}>
                {uniqMotion.map(m => (
                  <span key={m} style={{ fontSize:11, background:"rgba(10,132,255,0.12)",
                ...
                {uniqAudio.map(a => (
                  <span key={a} style={{ fontSize:11, background:"rgba(255,149,0,0.12)",
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:12 }}>
          {[
            ...
            <Card key={k} style={{ padding:"13px 16px" }}>
              <Lbl>{k}</Lbl>
              <div style={{ fontSize:22, fontWeight:800, color:"#e0e0e0",
...
// (Mock stream removed; UI is now driven purely by the real backend SSE stream.)